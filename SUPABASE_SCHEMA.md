...
-- Trips: members can see, owner can modify
...
-- Trip members: members of a trip can see all members
...
-- Expenses: trip members can read, insert own
...
-- Itinerary: all members can see, titular + owner can write
...
CREATE POLICY "itinerary_insert" ON public.itinerary_activities FOR INSERT
...
-- POIs: all members can read/write
...
CREATE POLICY "pois_insert" ON public.pois FOR INSERT
  WITH CHECK (auth.uid() = user_id);
